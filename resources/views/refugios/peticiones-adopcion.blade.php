                ...
                @foreach($familia->mascotas as $mascota)
                    @if($mascota->refugio->id == $refugio->id && !$mascota->adoptado)
                        <tr class="text-center">
                            ...
                            <td>
                                <a href="{{ route('mascotas.show' , $mascota) }}">{{$mascota->nombre}} {!! $mascota->especie->especie === "Perro" ? '<i class="fas fa-dog"></i>' : '<i class="fas fa-cat"></i>' !!}</a>
                            </td>
                            <td><a href="#"
                                   onclick="aceptarSolicitud('{{ $mascota->nombre }}', '{{ $familia->name }}', '{{asset("storage")}}/{{$mascota->imagen}}')">
